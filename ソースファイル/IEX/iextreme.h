...
public:
	...
	void Activate( );
	void Activate(int width,int height,int n,int f);

	bool Screentransformation(Vector3& Outp, const Vector3 Pos)
	{
		return Screentransformation(Outp, Pos, matView*matProjection);
	}
	bool Screentransformation(Vector3& Outp, const Vector3 Pos, const Matrix& mat);
};
...
public:
	...
	void float_Render(float DstX, float DstY, float DstW, float DstH, float SrcX, float SrcY, float width, float height, u32 dwFlags = RS_COPY, COLOR color = 0xFFFFFFFF, float z = .0f);
...
//		アクセス関数
//*****************************************************************************
void	KEY_Asign(KEYSET& padset, int i);
void	KEY_PadAsign(PADSET& padset, int i);

void	KEY_SetInfo(int i);
int		KEY_Get(KEYCODE key, int i);
void	KEY_Vibration(u32 gain, float period, int i);

int		KEY_GetAxisX(int i);
int		KEY_GetAxisY(int i);
int		KEY_GetAxisX2(int i);
int		KEY_GetAxisY2(int i);
