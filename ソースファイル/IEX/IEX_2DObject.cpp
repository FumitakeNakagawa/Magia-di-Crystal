...
void iex2DObj::float_Render(float DstX, float DstY, float DstW, float DstH, float SrcX, float SrcY, float width, float height, u32 dwFlags, COLOR color, float z)
{
	TLVERTEX	v[4];
	v[0].sx = v[2].sx = DstX;
	v[1].sx = v[3].sx = (DstX + DstW) - 0.5f;
	v[0].sy = v[1].sy = DstY;
	v[2].sy = v[3].sy = (DstY + DstH) - 0.5f;

	v[0].tu = v[2].tu = (SrcX + 0.5f) / (float)this->width;
	v[1].tu = v[3].tu = (SrcX + width) / (float)this->width;
	v[0].tv = v[1].tv = (SrcY + 0.5f) / (float)this->height;
	v[2].tv = v[3].tv = (SrcY + height) / (float)this->height;
